...
for url in urls:    
    stream = os.popen('lighthouse --quiet --no-update-notifier --screenEmulation.disabled=true --no-enable-error-reporting --output=json --output-path='+name+'_'+getdate+'.report.json --chrome-flags="--headless" ' + url +" " + '--form-factor=' + choic )
    time.sleep(90)
